...
def goal_test(board):
    ...
    startrange = 1
    endrange = 5
    if(board[0:1] != "."): #top wall
        startrange -=1
    if(board[7:8] != "."):
        endrange +=1
    for i in range(startrange, endrange): 
        if(board[i:i+1] == opptoken):
            if((endrange-i) >= 2 and board[i+1:i+4] == "..." and board[i+4:i+5] == opptoken):
                score+=4
            elif(board[i+1:i+2] == "." and board[i+2:i+3] == opptoken):
                score+=4
        elif(board[i:i+1] == token):
            if((endrange-i) >= 2 and board[i+1:i+4] == "..." and board[i+4:i+5] == token):
                score-=4
            elif(board[i+1:i+2] == "." and board[i+2:i+3] == token):
                score-=4

    startrange = 57
    endrange = 61  
    if(board[56:57] != "."): #bottom wall
        startrange -=1
    if(board[56:57] != "."):
        endrange +=1
    for i in range(startrange, endrange): 
    ...
    startrange = 8
    endrange = 33
    if(board[0:1] != "."): #leftwall
        startrange -= 8
    if(board[56:57] != "."):
        endrange+= 8
    for i in range(startrange, endrange, 8): 
        if(board[i:i+1] == opptoken):
            if((endrange-i) >= 16 and board[i+8:i+9] == "." and board[i+16:i+17] == "."
            and board[i+24:i+25] == "." and board[i+32:i+33] == opptoken):
                score+=4
            elif(board[i+8:i+9] == "." and board[i+16:i+17] == opptoken):
                score+=4
        elif(board[i:i+1] == token):
            if((endrange-i) >= 16 and board[i+8:i+9] == "." and board[i+16:i+17] == "."
            and board[i+24:i+25] == "." and board[i+32:i+33] == token):
                score-=4
            elif(board[i+8:i+9] == "." and board[i+16:i+17] == token):
                score-=4
