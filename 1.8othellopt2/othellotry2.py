...
def idminimax(startboard, token):
    maxdepth = 1
    result = []
    ...
    while(maxdepth < 5):
    # while(t2 - t1 < 2):
        temp = (idminimaxhelperval(startboard, maxdepth, 0, token))
        tempindex = idminimaxhelperindex(startboard, token, maxdepth, temp)
...
def idminimaxhelperval(board, maxdepth, depth, token):
    if(maxdepth == depth):
    ...
    if(depth < maxdepth):
        for child in othello_imports.possible_moves(board, token):
            ...
            temp = idminimaxhelperval(newboard, maxdepth, depth+1, opptoken)
            n = temp * -1
            results.append(n)
    return max(results)
...
def idminimaxhelperindex(startboard, token, maxdepth, value):
    ...
    for position in temp:
        ...
        if(idminimaxhelperval(next_board, maxdepth, 1, opptoken) == value * -1):
            return position
    return None
