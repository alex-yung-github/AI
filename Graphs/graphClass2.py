class Graph(object): #had ot make them lists because it is a directed graph and the arrows are only one way. Sets would make the arrows randomly pointed
    ...
    #def change_weight(self, start, end, weight): there is no weight in this graph
        
    def add_edge(self, edge): #made it so that it is only from edge element 1 to edge element 2 to instead of both ways
        ...
        if(temp2 not in self.graphD[temp1]):
            self.graphD[temp1].append(temp2)
...
print("Removing vertex 20")
graph.delete_vertex("20")
print("Vertices of graph:")
print(graph.all_vertices())
print("Edges of graph:")
print(graph.all_edges())